...
def list_search_terms(content_type: str):
    ...
    for keyword, date_info in search_history:
        ...
        AddMenuEntry(keyword, keyword, 130, icon, aired=date_created, context_mnu=ctx_mnu)
...
def new_search(content_type):
    ...
    if keyword:
        ...
        xbmc.executebuiltin(f'Container.Update(plugin://plugin.video.iplayerwww?mode=130&content_type={content_type}&url={keyword})')
...
def context_menu(content_type: str, action: str, keyword: str = ''):
